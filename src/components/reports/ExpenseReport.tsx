...
const ExpenseReport = ({ stockEntries, rawMaterials, expenseBreakdown, dateRange }: ExpenseReportProps) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-green-50 p-6 rounded-lg dark:bg-green-900/10 dark:border-green-700">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-green-600 dark:text-green-400">Total Purchases</p>
              <p className="text-2xl font-bold text-green-900 dark:text-green-300">${expenseBreakdown.totalPurchases.toFixed(2)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-blue-50 p-6 rounded-lg dark:bg-blue-900/10 dark:border-blue-700">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-blue-600 dark:text-blue-400">Average Order Value</p>
              <p className="text-2xl font-bold text-blue-900 dark:text-blue-300">${expenseBreakdown.averageOrderValue.toFixed(2)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-purple-50 p-6 rounded-lg dark:bg-purple-900/10 dark:border-purple-700">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-purple-900 dark:text-purple-300">{expenseData.totalEntries}</p>
            </div>
          </div>
        </div>

        <div className="bg-yellow-50 p-6 rounded-lg dark:bg-yellow-900/10 dark:border-yellow-700">
          <div className="flex items-center">
            <Users className="w-8 h-8 text-yellow-600 dark:text-yellow-400" />
            <div className="ml-4">
              <p className="text-sm font-medium text-yellow-600 dark:text-yellow-400">Active Suppliers</p>
              <p className="text-2xl font-bold text-yellow-900 dark:text-yellow-300">{expenseBreakdown.topSuppliers.length}</p>
            </div>
          </div>
        </div>
      </div>
            ...
            {expenseData.periods.map((period, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400">{period.count} orders</p>
                </div>
                <div className="text-right">
                  <p className="font-bold text-gray-900 dark:text-white">${period.total.toFixed(2)}</p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">Avg: ${period.count > 0 ? (period.total / period.count).toFixed(2) : "0.00"}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Top Suppliers */}
      <div className="bg-white p-6 rounded-lg border dark:bg-gray-800 dark:border-gray-700">
        <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">Top Suppliers</h3>
            ...
            <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
              {expenseData.supplierAnalysis.map((supplier, index) => (
                <tr key={supplier.name}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">${supplier.total.toFixed(2)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{supplier.orderCount}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">${supplier.avgOrderValue.toFixed(2)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{supplier.uniqueMaterials}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{supplier.lastOrder ? supplier.lastOrder.toLocaleDateString() : "N/A"}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="bg-white p-6 rounded-lg border dark:bg-gray-800 dark:border-gray-700">
        <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">Material Cost Analysis</h3>
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-800">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Avg Unit Cost</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Orders</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
              {expenseData.materialCosts.slice(0, 15).map(item => (
                <tr key={item.material.id}>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900 dark:text-white">{item.material.name}</div>
                    <div className="text-sm text-gray-500 dark:text-gray-400">{item.material.category}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">${item.totalCost.toFixed(2)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                    {item.totalQuantity} {item.material.unit}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">${item.avgUnitCost.toFixed(2)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{item.entryCount}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{item.lastPurchase ? item.lastPurchase.toLocaleDateString() : "N/A"}</td>
                </tr>
              ))}
            </tbody>
          </table>

    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-red-50 p-6 rounded-lg border border-red-200">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-red-600">Critical (Out of Stock)</p>
        ...
        <div className="bg-blue-50 p-6 rounded-lg border border-blue-200">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium text-blue-600">Total Low Stock</p>
              <p className="text-2xl font-bold text-blue-900">{lowStockItems.length}</p>
            </div>
          </div>
        </div>
      </div>
              ...
              {criticalItems.map(item => (
                <div key={item.rawMaterialId} className="flex items-center justify-between p-4 bg-red-50 rounded-lg border border-red-200">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">Category: {item.rawMaterial?.category}</p>
                  </div>
      ...
      {lowStockItems.length === 0 && (
        <div className="text-center py-12 bg-white rounded-lg border">
          ...
          <p className="text-gray-600">No items are currently below their minimum stock levels</p>
        </div>
      )}
    </div>
  );

...
const RecipesConsumptionModal = ({ section, recipe, isOpen, onClose, onSuccess, isHistoryView }: RecipesConsumptionModalProps) => {
  const { state } = useApp();
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`${isHistoryView ? "View" : "Record"} Consumption - ${recipe.name}`} size="xl">
      {inventoryIssues && (
